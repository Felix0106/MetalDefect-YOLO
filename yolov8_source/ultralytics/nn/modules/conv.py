...
class WeightedConcat(nn.Module):
    """Lightweight weighted concat fusion for two feature maps."""

    def __init__(self, dimension=1, n_inputs=2, eps=1e-4):
        super().__init__()
        self.d = dimension
        self.eps = eps
    ...
    def forward(self, x):
        # x: list[Tensor], expected length == n_inputs
        weight = torch.relu(self.w)
        weight = weight / (weight.sum() + self.eps)
        return torch.cat([xi * wi for xi, wi in zip(x, weight)], dim=self.d)
